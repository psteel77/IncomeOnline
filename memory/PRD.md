...
- **Dynamic platform landing pages for SEO** (P0 win): 185 unique individual platform URLs (`/platforms/{slug}`) now exist as crawlable pages with full SEO — per-page title, meta description, canonical link, Open Graph, Twitter card, and JSON-LD Product schema with aggregateRating. Backend emits `/api/seo/platforms-sitemap.xml` (XML sitemap of all platform URLs), `/api/seo/sitemap-index.xml` (index referencing both main + platforms sitemap), and `/api/seo/platform-by-slug/{slug}` (data endpoint). `robots.txt` updated to advertise both sitemaps. SEO tags applied via custom `useSEO()` hook (React Helmet Async v2 was silently dropping meta/link mergers in React 18 — imperative hook bypasses the issue and cleanly restores on unmount).
