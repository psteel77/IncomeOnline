...
- **Automated 7-day renewal email (18 Jul) [PENDING DEPLOY]:** the existing `send_expiry_warning_email` ("Renew now →", £9.99/12 more months) + `POST /subscription/process-expirations` were never scheduled. Added a **daily APScheduler job** (`_expiry_job`, `EXPIRY_INTERVAL_HOURS=24`, `EXPIRY_SCHEDULER_ENABLED=true`) to the existing in-process scheduler; refactored `start_recovery_scheduler` to start if recovery OR expiry is enabled. Idempotent via `warning_sent` flag (reset on renewal in `_upsert_donor`). Verified in preview: warnings_sent=1 then 0 on re-run; scheduler logs "[expiry-cron] scheduled · every 24h".
...
## 2026-06 — Bold text + Blank branded template (DONE)
- Fixed weak/light text: Montserrat **variable** font rendered Regular in LibreOffice and ignored the bold flag. Installed static Montserrat faces (Regular/Bold/Italic/BoldItalic/SemiBold from Fontsource). Set Normal + list styles + body/callout/table runs to bold; darkened BODY_TEXT to #141418. Regenerated ALL 15 guide .docx and rebuilt every PDF + Premium Pack — text is now bold and high-contrast.
- New blank branded template: `backend/generate_template_doc.py` → `static/MoneyRules_Branded_Template.docx/.pdf`. Skeleton only (title page, branded chapter panels each on a new page, one of every callout with placeholder text, closing page). Served at `GET /api/pdf/branded-template` (Word) and `GET /api/pdf/branded-template-pdf` (PDF). Verified 200.
- NOTE for future builds: static Montserrat faces must be installed at build time (Fontsource latin-400/600/700 normal+italic). Do NOT use the variable Montserrat[wght].ttf — LibreOffice renders it Regular and bold is lost.
