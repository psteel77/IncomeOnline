...
- Verified in preview: 20/20 premium-pack tests pass; payload build unit-checked (plain + attachment + headers); no-token path returns False gracefully. **NEEDS: user to set POSTMARK_SERVER_TOKEN on Railway + confirm Postmark sender signature for welcome@incomeonline.info, then Save to Github.**
